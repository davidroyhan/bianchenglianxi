...
{
	RTIME now, previous,t_speed,t_stay,t_decrease,time,pre_time;
	...
	while (1) {
		 ...
		 */
             ...
             if(new_cmd.Request){
                ...
                axis1_setpoint.Position+=axis1_setpoint.Velocity*1;
                if(axis1_setpoint.Position<=new_cmd.Velocity*new_cmd.Velocity/(2*new_cmd.Acceleration)){
                 axis1_setpoint.Velocity+=new_cmd.Acceleration*1;}
                else if(axis1_setpoint.Position<=new_cmd.Position-new_cmd.Velocity*new_cmd.Velocity/(2*new_cmd.Acceleration)){
                        axis1_setpoint.Velocity=axis1_setpoint.Velocity;}
                else if(axis1_setpoint.Position<=new_cmd.Position){
                 axis1_setpoint.Velocity-=new_cmd.Acceleration*1;}      
              }
